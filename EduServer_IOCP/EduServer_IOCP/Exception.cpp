...
LONG WINAPI ExceptionFilter(EXCEPTION_POINTERS* exceptionInfo)
{
	...
	if (hThreadSnap != INVALID_HANDLE_VALUE)
	{
		...
		if (Thread32First(hThreadSnap, &te32))
		{
			do
			{
				...
				if( te32.th32ThreadID != myThreadId && te32.th32OwnerProcessID == myProcessId ){
	...
	StringCbPrintf( tszFileName,
		_countof( tszFileName ),
		_T( "%s_%4d%02d%02d_%02d%02d%02d.txt" ),
		_T( "EduServer_exception" ),
		stTime.wYear,
		stTime.wMonth,
		stTime.wDay,
		stTime.wHour,
		stTime.wMinute,
		stTime.wSecond );
	std::ofstream historyOut( tszFileName, std::ofstream::out );
